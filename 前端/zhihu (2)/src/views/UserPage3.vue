    ...
    <div class="body">
        <div id="HeadBackground" >
               ...
               <div v-bind:class="showBigHeader?'bigHeader':'header'" id="Header-down">
                   <div class="Bussiness" v-if="UserData.business!=null"><a id="Bussiness" >公司:{{UserData.business}}</a></div>
                   <div  class="Gender" v-if="UserData.gender!=null"><a id="Gender">用户性别:{{UserData.gender}}</a></div>
                   <div  class="Description" v-if="UserData.description!=null"> <a id="Description">{{UserData.description}}</a></div>
                   <div class="Headline" v-if="UserData.headline!=null"> <a id="Headline" >{{UserData.headline}}</a> </div>
                   <div class="Location" v-if="UserData.location!=null"> <a id="Location" >居住地:{{UserData.location[0]}}</a> </div>
               </div>
    ...
    export default {
        ...
        methods: {
            ...
            async GetUserData() {
                ...
                //获取用户个人数据
               await axios.get('http://101.34.151.80:9090/user_check/getUserById2?id2=5ea6b09b624e11ffef62db3632512bd6').then((res)=>{
                ...
                axios.post("http://101.34.151.80:9090/all_figure/get_userDraw",
                    {
                        ...
                        for (let i=0;i<temporary.length;i++){
                            if (temporary[i].key_value1.match("上海")){
            ...
            async start(){
                ...
                await this.GetLocationGender()
                await this.GetLocationLevel()
                // await this.GetLocationLevel()


                await this.GetAllGenderLevel()
                await this.GetLevelMessage()
            },
